...
namespace System.Framework
{
	...
	public class GlProgramHandle : OpenGLHandle
	{
		protected GlProgramHandle(string strName)
			: base("Programm_" + strName)
		{
			m_pHandle = gl.glCreateProgramObjectARB();
	...
	public class Program : GlProgramHandle
	{
		...
		{
			gl.glAttachObjectARB(m_pHandle, shader.glObject);
		}
		public void Link()
		{
			int res = 0;

			gl.glLinkProgramARB(m_pHandle);
			gl.glGetProgramiv( m_pHandle, (uint)GL.LINK_STATUS, ref res );
		...
		{
			int bufSize = 0;
			gl.glGetShaderiv( m_pHandle, (uint)GL.INFO_LOG_LENGTH, ref bufSize );

			return (bufSize > 0 ? gl.glGetProgramInfoLogARB(m_pHandle, bufSize) : "");
		}
		public void Use()
		{
			gl.glUseProgramObjectARB(m_pHandle);
		}
		...
		{
			return gl.glGetAttribLocation( m_pHandle, name );
		}
		public int Uniform( string name )
		{
			return gl.glGetUniformLocationARB(m_pHandle, name);
		}
