...
namespace X11.Widgets
{
	...
	public class Screen : XHandle
	{
		...
		public int 		DefaultColormap
		{
			get
			{
				return X11._internal.Lib.XDefaultColormapOfScreen (m_pHandle);
			}
		}
			...
			: base(name + iScreenNumber)
		{
			...
			m_pHandle = X11._internal.Lib.XScreenOfDisplay(display.RawHandle, 
														  (TInt)iScreenNumber);

			m_iScreenNumber = iScreenNumber;
			m_iScreenWidth = (int)X11._internal.Lib.XDisplayWidth(display.RawHandle, 
				(TInt)m_iScreenNumber);
			m_iScreenHeight = (int)X11._internal.Lib.XDisplayHeight(display.RawHandle, 
				(TInt)m_iScreenNumber);
			...
			: base("SCREEN")
		{
			...
			m_pHandle = screen;
			m_iScreenNumber = X11._internal.Lib.XScreenNumberOfScreen(m_pHandle);
